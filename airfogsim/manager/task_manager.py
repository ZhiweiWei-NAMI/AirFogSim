...
class TaskManager:
    ...
    def _generateTaskInfo(self, task_node_id, arrival_time):
        self._task_id += 1
        return Task(task_id = self._task_id, task_node_id = task_node_id, task_cpu = self._generateCPU(), task_size = self._generateSize(), task_deadline = self._generateDeadline(), task_priority = self._generatePriority(), task_arrival_time = arrival_time)
    ...
    def _generateTasks(self, task_node_ids, cur_time, generation_interval):
        ...
        for node_id, task_infos in self._to_generate_task_infos.items():
            for task_info in task_infos.copy():
                if task_info['generation_time'] <= cur_time:
                    todo_task_list = self._todo_tasks.get(node_id, [])
                    todo_task_list.append(task_info)
                    self._todo_tasks[node_id] = todo_task_list
    ...
    def _checkTasks(self, cur_time):
        ...
        for node_id, task_infos in self._todo_tasks.items():
            for task_info in task_infos.copy():
                if task_info.getTaskDeadline() + task_info.getTaskArrivalTime() <= cur_time:
                    task_info.setTaskFailueReason('Deadline missed.')
                    ...
                    self._todo_tasks[node_id].remove(task_info)