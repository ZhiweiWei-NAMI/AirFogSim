...
class DDPG_Agent:
    def __init__(self, args):
        ...
        self.tau = args.tau
        # state包括task_node, task_data, compute_node
        self.state_dim = self.d_node * self.m1 + self.d_task * self.max_tasks * self.m1 + self.d_node * self.m2
    ...
    def select_action(self, task_node, task_data, compute_node, task_mask, compute_node_mask):
        ...
        state = torch.cat([task_node.view(-1), task_data.view(-1), compute_node.view(-1)], dim=0).unsqueeze(0)
    ...
    def update(self, writer):
        ...
        state = torch.cat([task_node.view(batch_size, -1), task_data.view(batch_size, -1), compute_node.view(batch_size, -1)], dim=1)
        next_state = torch.cat([next_task_node.view(batch_size, -1), next_task_data.view(batch_size, -1), next_compute_node.view(batch_size, -1)], dim=1)
        ...
        with torch.no_grad():
            ...
            target_q = reward + (1 - done) * self.gamma * target_q.squeeze()
